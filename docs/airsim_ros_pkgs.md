...
## Running
In the launch file `airsim_node.launch` set the `vehicle_type` parameter to
either `car` or `multirotor`. It should match the sim mode that is set in
`~/Documents/Airsim/settings.json`. Also if you are using the old AirSim Linux
binaries set the parameter `is_vulkan` to `true` to ensure image encodings are
handled correctly.

 1. Run AirSim either using the binaries or using the UnrealEditor if you have
 built the Unreal Engine. Make sure to modify the AirSim `settings.json` file
 to match your preferred setup. A sample settings file is provided here for the
 car sim mode and you can use it by running the following command:
 ```
 cp src/airsim_tutorial_pkgs/settings/settings_light_mono_lidar_car.json ~/Documents/AirSim/settings.json
 ```
...
The ROS wrapper is composed of two ROS nodes - the first is a wrapper over AirSim's multirotor and car C++ client libraries, and the second is a simple PD position controller.    
...
#### Subscribers:
- `/airsim_node/VEHICLE_NAME/cmd_vel` [geometry_msgs/Twist](http://docs.ros.org/melodic/api/geometry_msgs/html/msg/Twist.html)
  Use this topic in car sim mode to send velocity commands to the car.
...
### Notes
- Currently there are issues with receiving the IMU readings. Do not set
an IMU in the sensor list within the `settings.json`.
