...
void AirsimROSWrapper::add_ros_car(const std::string& vehicle_name) {
  ...
  car_ros.vel_cmd_body_frame_cb_std_sub = 
    ...
    "/cmd_vel", 1, 
      boost::bind(&AirsimROSWrapper::vel_cmd_body_frame_std_cb, this, _1, 
...
void AirsimROSWrapper::add_ros_multirotor(const std::string& vehicle_name) {
  ...
  multirotor_ros.takeoff_srvr = 
      ...
      airsim_ros_pkgs::Takeoff::Response>(vehicle_name + "/takeoff", 
  ...
  multirotor_ros.land_srvr = 
      ...
      airsim_ros_pkgs::Land::Response>(vehicle_name + "/land", 
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    if (vehicle_type_ == MULTIROTOR) {
      gimbal_angle_quat_cmd_sub_ = 
              nh_private_.subscribe("gimbal_angle_quat_cmd", 50, 
              &AirsimROSWrapper::gimbal_angle_quat_cmd_cb, this);
      gimbal_angle_euler_cmd_sub_ = 
              nh_private_.subscribe("gimbal_angle_euler_cmd", 50, 
    ...
    if (!((sim_mode == "Car" &&
         ...
         (sim_mode == "Multirotor" &&
         vehicle_type_ == MULTIROTOR ))) {
      LOG(FATAL) << "Sim mode that is set in the settings.json("
                 << sim_mode << ") does not match the vehicle_type set for the"
                 << " ROS wrapper.";
    }
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
    {
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras)
        {
            ...
            append_static_camera_tf(curr_vehicle_name, curr_camera_name, 
camera_setting);
...
current_image_request_vec.push_back(ImageRequest(curr_camera_name, 
curr_image_type, false, false));
                    }
...
current_image_request_vec.push_back(ImageRequest(curr_camera_name, 
curr_image_type, true));
                    }

                    
image_pub_vec_.push_back(image_transporter.advertise(curr_vehicle_name + "/" + 
curr_camera_name + "/" + 
...
image_type_int_to_string_map_.at(capture_setting.image_type) + "/camera_info", 
10));
                    
camera_info_msg_vec_.push_back(generate_cam_info(curr_camera_name, 
camera_setting, capture_setting));
                }
            }
            // push back pair (vector of image captures, current vehicle name) 
            
airsim_img_request_vehicle_name_pair_vec_.push_back(std::make_pair(
current_image_request_vec, curr_vehicle_name));
        ...
        for (auto& curr_sensor_map : vehicle_setting->sensors)
        {
            ...
            switch (sensor_setting->sensor_type)
            {
                ...
                case SensorBase::SensorType::Lidar:
                {
                    ...
                    auto lidar_setting = *static_cast<LidarSetting*>(
                                               sensor_setting.get());
    ...
    {
        takeoff_all_srvr_ = nh_private_.advertiseService("all_robots/takeoff", 
&AirsimROSWrapper::takeoff_all_srv_cb, this);
        land_all_srvr_ = nh_private_.advertiseService("all_robots/land", 
&AirsimROSWrapper::land_all_srv_cb, this);
...
&AirsimROSWrapper::takeoff_group_srv_cb, this);
        land_group_srvr_ = nh_private_.advertiseService("group_of_robots/land", 
&AirsimROSWrapper::land_group_srv_cb, this);
    }

    // todo add per vehicle reset in AirLib API
    reset_srvr_ = 
nh_private_.advertiseService("reset",&AirsimROSWrapper::reset_srv_cb, this);
...
update_airsim_img_response_every_n_sec);
        ...
        {
            ros::TimerOptions 
timer_options(ros::Duration(update_airsim_img_response_every_n_sec), 
...
&img_timer_cb_queue_);
        ...
        {
            airsim_img_response_timer_ = 
nh_private_.createTimer(ros::Duration(update_airsim_img_response_every_n_sec), 
&AirsimROSWrapper::img_response_timer_cb, this);
    ...
    {    
        ...
        nh_private_.getParam("update_lidar_every_n_sec", 
update_lidar_every_n_sec);
        ...
        {
            ros::TimerOptions 
timer_options(ros::Duration(update_lidar_every_n_sec), 
...
ros::Time AirsimROSWrapper::make_ts(uint64_t unreal_ts) {
    ...
    return  first_imu_ros_ts + ros::Duration( (unreal_ts- 
first_imu_unreal_ts)/1e9);
}


// todo: error check. if state is not landed, return error. 
bool AirsimROSWrapper::takeoff_srv_cb(airsim_ros_pkgs::Takeoff::Request& 
request, airsim_ros_pkgs::Takeoff::Response& response, const std::string& 
vehicle_name)
{
...
bool 
AirsimROSWrapper::takeoff_group_srv_cb(airsim_ros_pkgs::TakeoffGroup::Request& 
...
bool AirsimROSWrapper::takeoff_all_srv_cb(airsim_ros_pkgs::Takeoff::Request& 
request, airsim_ros_pkgs::Takeoff::Response& response)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
airsim_ros_pkgs::Land::Response& response, const std::string& vehicle_name)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_group_srv_cb(airsim_ros_pkgs::LandGroup::Request& 
...
bool AirsimROSWrapper::land_all_srv_cb(airsim_ros_pkgs::Land::Request& request, 
...
tf2::Quaternion AirsimROSWrapper::get_tf2_quat(const msr::airlib::Quaternionr& 
airlib_quat) const
{
...
msr::airlib::Quaternionr AirsimROSWrapper::get_airlib_quat(const 
geometry_msgs::Quaternion& geometry_msgs_quat) const
{
...
airsim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
    ...
    // ros uses xyzw
tf2::Matrix3x3(get_tf2_quat(multirotor_ros_vec_[vehicle_idx].curr_drone_state.
kinematics_estimated.pose.orientation)).getRPY(roll, pitch, yaw); 

    // todo do actual body frame?
    //body frame assuming zero pitch roll
    multirotor_ros_vec_[vehicle_idx].vel_cmd.x = (msg->twist.linear.x * 
cos(yaw)) - (msg->twist.linear.y * sin(yaw)); 
    multirotor_ros_vec_[vehicle_idx].vel_cmd.y = (msg->twist.linear.x * 
sin(yaw)) + (msg->twist.linear.y * cos(yaw)); //body frame
...
// TODO(Sadegh): Update this to support cars
void AirsimROSWrapper::vel_cmd_group_body_frame_cb(const 
...
void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const airsim_ros_pkgs::VelCmd& 
msg)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
// TODO(Sadegh): Update this to support cars
void AirsimROSWrapper::vel_cmd_world_frame_cb(const 
airsim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_world_frame_cb(const 
airsim_ros_pkgs::VelCmdGroup& msg)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_all_world_frame_cb(const airsim_ros_pkgs::VelCmd& 
msg)
{
    std::lock_guard<std::recursive_mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::gimbal_angle_quat_cmd_cb(const 
airsim_ros_pkgs::GimbalAngleQuatCmd& gimbal_angle_quat_cmd_msg)
{
...
void AirsimROSWrapper::gimbal_angle_euler_cmd_cb(const 
airsim_ros_pkgs::GimbalAngleEulerCmd& gimbal_angle_euler_cmd_msg)
{
    try
    {
        tf2::Quaternion quat_control_cmd;
...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const 
...
char*>(&data_std[0]);
        vector<unsigned char> lidar_msg_data(bytes, bytes + sizeof(float) * 
data_std.size());
...
// todo covariances
sensor_msgs::Imu AirsimROSWrapper::get_imu_msg_from_airsim(const 
...
AirsimROSWrapper::get_gps_sensor_msg_from_airsim_geo_point(const 
...
lck(drone_control_mutex_);
                airsim_client_.moveByVelocityAsync(multirotor_ros.vel_cmd.x, 
multirotor_ros.vel_cmd.y, multirotor_ros.vel_cmd.z, vel_cmd_duration_, 
                    msr::airlib::DrivetrainType::MaxDegreeOfFreedom, 
multirotor_ros.vel_cmd.yaw_mode, multirotor_ros.vehicle_name);
                lck.unlock();
            }
...
lck(drone_control_mutex_);
                auto imu_data = 
airsim_client_.getImuData(vehicle_imu_pair.second, vehicle_imu_pair.first);
                lck.unlock();
...
gimbal_cmd_.target_quat, gimbal_cmd_.vehicle_name);
    ...
    catch (rpc::rpc_error& e)
    {
        ...
        std::string msg = e.get_error().as<std::string>();
        std::cout << "Exception raised by the API:" << std::endl << msg << 
std::endl;
    }
}
...
//         car_ros.global_gps_pub.publish(car_ros.gps_sensor_msg);
        ...
        if (car_ros.has_vel_cmd) {
            ...
            // TODO(Kavan): send control commands to car (use 
            // airsim_car_client_)
            double currentVel = 
              car_ros.curr_car_state.kinematics_estimated.twist.linear.x();
            ...
            if (currentVel < targetVel - CAR_VEL_EPSILON) {
                ...
                // Only turn off the vel cmd once we have achieved target 
                // velocity
...
car_ros.curr_car_state.kinematics_estimated.twist.angular.z();
    ...
    if (imu_pub_vec_.size() > 0)
    {
      ...
      for (const auto& vehicle_imu_pair: vehicle_imu_map_)
      {
        std::unique_lock<std::recursive_mutex>lck(drone_control_mutex_);
        auto imu_data = 
        airsim_car_client_.getImuData(vehicle_imu_pair.second, 
...
// handling tfs in ROS 
void AirsimROSWrapper::set_nans_to_zeros_in_pose(VehicleSetting& 
vehicle_setting) const
{
...
void AirsimROSWrapper::set_nans_to_zeros_in_pose(const VehicleSetting& 
vehicle_setting, CameraSetting& camera_setting) const
{
...
void AirsimROSWrapper::set_nans_to_zeros_in_pose(const VehicleSetting& 
vehicle_setting, LidarSetting& lidar_setting) const
{
...
void AirsimROSWrapper::append_static_vehicle_tf(const std::string& vehicle_name, 
const VehicleSetting& vehicle_setting)
{
    ...
    tf2::Quaternion quat;
    quat.setRPY(vehicle_setting.rotation.roll, vehicle_setting.rotation.pitch, 
...
void AirsimROSWrapper::append_static_lidar_tf(const std::string& vehicle_name, 
const std::string& lidar_name, const LidarSetting& lidar_setting)
{

    geometry_msgs::TransformStamped lidar_tf_msg;
    lidar_tf_msg.header.frame_id = vehicle_name + frame_name_base_link; // todo 
                                                            // multiple drones
    ...
    tf2::Quaternion quat;
    quat.setRPY(lidar_setting.rotation.roll, lidar_setting.rotation.pitch, 
...
void AirsimROSWrapper::append_static_camera_tf(const std::string& vehicle_name, 
      ...
      if (lidar_pub_vec_.size() > 0)
      {
        ...
        for (const auto& vehicle_lidar_pair: vehicle_lidar_map_)
        {
          ...
          switch (vehicle_type_) {
            case CAR: {
              auto lidar_data = 
              airsim_car_client_lidar_.getLidarData(vehicle_lidar_pair.second, 
                                                vehicle_lidar_pair.first); 
            ...
            case MULTIROTOR: {
              auto lidar_data = 
                airsim_client_lidar_.getLidarData(vehicle_lidar_pair.second, 
                                                  vehicle_lidar_pair.first); 
              lck.unlock();
              // todo make const ptr msg to avoid copy
              lidar_msg = get_lidar_msg_from_airsim(lidar_data);  
